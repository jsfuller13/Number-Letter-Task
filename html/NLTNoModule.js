...
function experimentInit() {
  ...
  gen_instr_2_txt = new visual.TextStim({
    ...
    text: 'In this first block, pairs will appear on the top half of the screen.\n\nDecide if the number is odd or even.',
    font: 'Arial',
    units : undefined, 
    pos: [0, 0], height: 0.05,  wrapWidth: undefined, ori: 0,
    color: new util.Color('white'),  opacity: 1,
...
function Gen_InstrRoutineBegin() {
  ...
  Gen_InstrComponents.forEach( function(thisComponent) {
...
function gen_instr_2RoutineBegin() {
  ...
  gen_instr_2Components.forEach( function(thisComponent) {
    if ('status' in thisComponent)
      thisComponent.status = PsychoJS.Status.NOT_STARTED;
     });
